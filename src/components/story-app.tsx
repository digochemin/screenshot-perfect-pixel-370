...
function Choice({ children, state, onClick }: { children: React.ReactNode; state?: "right" | "wrong"; onClick: () => void }) {
...
export function StoryApp() {
  ...
  return <main className="story-shell">
    ...
    <AnimatePresence mode="wait" custom={direction}>
      <motion.article key={screen} custom={direction} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, x: direction * 60 }} animate={{ opacity: 1, x: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, x: direction * -60 }} transition={{ duration: .35 }} className={`story-screen ${[1, 5, 6, 12, 13, 14, 15, 16, 17, 18, 22, 23, 29, 30, 31].includes(screen) ? "story-dark" : "story-cream"}`}>
...
function renderScreen(screen: number, lead: LeadSession, selected: Record<number, number>, answer: (index: number, correct: number) => void, answers: AnswerMap, ticket: number | null, setTicket: (value: number) => void) {
  ...
  if (screen >= 8 && screen <= 11) { const card = triggerCards[screen - 8]; const name = card[0] as Trigger; return <div className="trigger-card"><div className={triggerColors[name]}><span>{screen - 7}</span><h2>{name}</h2><small>{card[1]}</small></div><p>{card[3]}</p><b className={`reaction ${triggerColors[name]}`}>GERA: {card[2]}</b><ul>{card[4].map((x) => <li key={x}>{personalize(x, lead)}</li>)}</ul>{screen === 8 && <aside>⚠️ Polêmica não é ofensa. Opinião forte sobre jogador, escalação e decisão de jogo funciona. Ataque pessoal, família e questões que não têm a ver com futebol destroem a página. A linha é essa.</aside>}</div>; }
  ...
  if (screen === 22) { const pick = selected[22]; return <><span className="story-badge">SUA VEZ</span><Title>A página “Resenha {lead.team}” está perdendo alcance toda semana. O que está errado?</Title><div className="week-strip">SEG P · TER P · QUA — · QUI — · SEX P · SÁB P · DOM —</div><p className="fake-comment">“Essa página só sabe reclamar...”</p><div className="round-options">{["Está postando pouco", "Só usa um gatilho e não tem rotina", "Deveria postar mais fotos bonitas", "Precisa aparecer em vídeo"].map((x, i) => <Choice key={x} onClick={() => answer(i, 1)} state={pick === undefined ? undefined : i === 1 ? "right" : i === pick ? "wrong" : undefined}>{x}</Choice>)}</div>{pick !== undefined && <Why>Ela só usa polêmica e posta sem padrão. Resultado: só ganha um tipo de reação, a audiência cansa, e a página fica com fama de só reclamar. Veja a mesma página com a semana dos 4 gatilhos: SEG Polêmica / QUA Nostalgia / SEX Revelação / DOM Pertencimento.</Why>}</>; }
  ...
  ][screen - 25]; return <><div className="source-heading"><i>{screen - 24}</i><div><small>{items[1]}</small><Title>{items[0]}</Title></div></div><p>{items[2]}</p><div className="point"><b>O ponto:</b> {items[3]}</div><div className="mini-stairs"><span className={screen === 27 ? "on" : ""}>INDEPENDÊNCIA</span><span className={screen === 26 ? "on" : ""}>VOLUME</span><span className={screen === 25 ? "on" : ""}>BASE</span></div></>; }
