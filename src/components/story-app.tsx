...
export function StoryApp() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setScreen(Math.min(30, Math.max(1, progress.screen ?? 1)));
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void supabase.rpc("update_lead_story_progress", {
        ...
        p_concluiu: screen === 30,
      });
    }, 300);
    if (screen === 30) fire("ConcluiuIsca");
    return () => window.clearTimeout(timer);
  }, [answers, lead, screen, ticket]);

  const locked = [5, 14, 15, 16, 17, 22, 28].includes(screen) && selected[screen] === undefined && !(screen === 28 && ticket);
  function go(delta: number) {
    if (delta > 0 && (locked || screen === 30)) return;
    setHint(false); setDirection(delta); setScreen((value) => Math.max(1, Math.min(30, value + delta)));
  }
  useEffect(() => {
  ...
  return <main className="story-shell">
    <div className="story-progress" aria-label={`Tela ${screen} de 30`}>{Array.from({ length: 30 }, (_, i) => <span key={i} className={i < screen ? "active" : ""} />)}</div>
    ...
    <AnimatePresence mode="wait" custom={direction}>
      <motion.article key={screen} custom={direction} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, x: direction * 60 }} animate={{ opacity: 1, x: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, x: direction * -60 }} transition={{ duration: .35 }} className={`story-screen story-screen-${screen} ${[1, 5, 6, 12, 13, 14, 15, 16, 17, 18, 22, 28, 29, 30].includes(screen) ? "story-dark" : "story-cream"}`}>
...
function renderScreen(screen: number, lead: LeadSession, selected: Record<number, number>, answer: (index: number, correct: number) => void, answers: AnswerMap, ticket: number | null, setTicket: (value: number) => void) {
  ...
  if (screen === 22) { const pick = selected[22]; return <><span className="story-badge">SUA VEZ</span><Title>Uma marca quer anunciar. Qual página vale mais pra ela?</Title><div className="profile-grid"><Choice onClick={() => answer(0, 1)} state={pick === undefined ? undefined : "wrong"}><span><b>PÁGINA A</b>200 mil seguidores<small>40 comentários por post</small></span></Choice><Choice onClick={() => answer(1, 1)} state={pick === undefined ? undefined : "right"}><span><b>PÁGINA B</b>30 mil seguidores<small>900 comentários por post</small></span></Choice></div>{pick !== undefined && <Why>Marca não compra seguidor. Compra gente prestando atenção. A página B tem uma audiência que reage — e é essa audiência que clica, compra e lembra da marca. E percebe? Tudo que você aprendeu até aqui é sobre gerar exatamente isso.</Why>}</>; }
  if (screen === 23) return <><Title>AGORA SIM: COMO<br/><Mark>ATENÇÃO VIRA DINHEIRO</Mark></Title><p>Existem várias formas de monetizar futebol. Mas três respondem pela maior parte do faturamento de quem vive disso. É nelas que você deve focar.</p><div className="money-stairs"><div>INDEPENDÊNCIA <Flag /></div><div>VOLUME</div><div>BASE</div></div></>;
  if (screen >= 24 && screen <= 26) { const items = [
  ...
  ][screen - 24] ?? ["MONETIZAÇÃO POR VISUALIZAÇÃO", "BASE", "", ""]; return <><div className="source-heading"><i>{screen - 23}</i><div><small>{items[1]}</small><Title>{items[0]}</Title></div></div><p>{items[2]}</p><div className="point"><b>O ponto:</b> {items[3]}</div><div className="mini-stairs"><span className={screen === 26 ? "on" : ""}>INDEPENDÊNCIA</span><span className={screen === 25 ? "on" : ""}>VOLUME</span><span className={screen === 24 ? "on" : ""}>BASE</span></div></>; }
  if (screen === 27) return <><Title>A LÓGICA DAS TRÊS</Title><div className="money-stairs big"><div>INDEPENDÊNCIA <Flag /></div><div>VOLUME</div><div>BASE</div></div><div className="yellow-callout">A lógica das três: a primeira te dá base, a segunda te dá volume, a terceira te dá independência. Quem fatura alto não escolhe uma — empilha as três.</div></>;
  if (screen === 28) { const values: Record<number, [string, number]> = { 27: ["3,7 milhões", 371], 97: ["1,04 milhão", 104], 297: ["337 mil", 34], 997: ["101 mil", 11] }; const result = ticket ? values[ticket] : null; return <><span className="story-badge">SUA VEZ</span><Title>A CONTA QUE<br/><span className="text-primary">FECHA EM R$ 10K</span></Title><p>R$ 10 mil parece distante quando você não tem uma estratégia e nem um funil. Deixa de parecer quando você entende o jogo.</p><p>Na vida real, de cada 10 mil pessoas que veem seu conteúdo, cerca de 1 compra (0,01%). Quantas visualizações você precisa pra faturar R$ 10 mil? Depende do preço do que você vende. Escolha:</p><div className="ticket-row">{[27, 97, 297, 997].map((x) => <Button key={x} onClick={() => { setTicket(x); fire("Respondeu", { numero: 28, acertou: true }); }} variant={ticket === x ? "game" : "outline"}>R$ {x}</Button>)}</div>{result && <><div className="funnel"><div><Eye /> <b>{result[0]}</b><small>VISUALIZAÇÕES/MÊS</small></div><div><ShoppingCart /> <b>{result[1]}</b><small>VENDAS A 0,01%</small></div><div><b>R$ {ticket}</b><small>TICKET</small></div><strong>= R$ 10.000 BRUTO</strong></div><Why>Percebe a diferença? Com um produto de R$ 27 você precisa de milhões de visualizações. Com um de R$ 297, uma página média já chega lá. Quem fatura alto não depende de viralizar todo dia — tem produtos com valor de verdade.</Why></>}<small className="disclaimer">Exemplo hipotético, não uma promessa de resultado. Alcance e conversão variam. O cálculo não desconta custos, taxas ou impostos.</small></>; }
  if (screen === 29) return <><Title>O QUE VOCÊ LEVA DAQUI</Title><Sequence className="takeaways">{["Futebol cresce com reação, não com beleza", "Os 4 gatilhos e a reação que cada um gera", "Escolher o gatilho pelo objetivo do post", "A semana dos 4 gatilhos", "Engajamento e ticket fecham a conta, não seguidor"].map((x) => <p key={x}><Check />{x}</p>)}</Sequence><div className="first-posts"><b>SEUS PRIMEIROS 4 POSTS</b>{rounds.map((r, i) => <p key={r.news}><span className={Object.values(triggerColors)[i]}>{r.options[0][0]}</span>{personalize(r.options[0][1], lead)}</p>)}<small>Pode usar. São seus.</small></div></>;
